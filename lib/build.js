...
var build = function(symbol, modules, dependencies) {	
	...
	dependencies.each(function(id, symbol) {
		//TODO change this based on packaging type 
		src += "\trequire.define('"+ id + "', function(require, module) { module.exports = root." + symbol + "; });\n"; 
	});
	...
	for(i = 0, len = modules.length; i < len; i++) {
		module = modules[i];
		src += "\trequire.define('" + module.id + "', function(require, module) {\n\t\t" + module.src.replace(/\n/g, '\n\t\t') + "\n\t});\n\n";
	}
	
	//TODO change this based on packaging type?
	//export the main module onto the root
	module = _.first(modules);
	src += "\troot." + symbol + " = require('" + module.id + "');\n})(this);";
